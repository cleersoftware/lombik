...
from application import introspection
...
def _model_index() -> dict[str, str]:
    return {m["table_name"]: m["class_name"] for m in introspection.list_models()}
...
def _label_attr(class_name: str) -> str:
    path = introspection.resolve_model_file(class_name)
    if path:
        info = introspection.parse_model_file(path)
        if info:
...
_SERVICES_TEMPLATE = '''from datetime import date, datetime, time
import json
...
_FORMS_TEMPLATE = '''from application.forms import Form, InputField, SelectField, CheckboxField, TextareaField
from .services import _related_options
__RELATED_IMPORTS__
__ENUM_IMPORTS__
...
from .services import (
    ...
    get_all___MODULE__,
    get___MODEL_VAR__,
    create___MODEL_VAR__,
    update___MODEL_VAR__,
    delete___MODEL_VAR__,
)
...
<section class="w-full h-full px-8 py-8">
    <div class="max-w-6xl mx-auto">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-bold text-ink">__CLASS__</h1>
                <p class="text-sm text-ink-muted mt-1">__TABLE__</p>
            </div>
            <a href="{{ url_for('__BP__.new') }}"
               class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-ink text-canvas text-sm font-medium hover:opacity-90 transition">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-line">
                        {% for item in items %}
                        <tr class="hover:bg-line/50 transition">
                            {% for col in columns %}
                            <td class="px-4 py-3 text-ink-muted truncate">{{ item[col.name] }}</td>
                            {% endfor %}
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ url_for('__BP__.detail', item_id=item.id) }}" class="text-ink-muted hover:text-brand transition" title="View">
                                        <ion-icon name="eye-outline" class="text-lg"></ion-icon>
                                    </a>
                                    <a href="{{ url_for('__BP__.edit', item_id=item.id) }}" class="text-ink-muted hover:text-brand transition" title="Edit">
                                    ...
                                    <form method="POST" action="{{ url_for('__BP__.delete', item_id=item.id) }}"
                                        ...
                                        <button type="submit" class="text-ink-muted hover:text-danger transition" title="Delete">
...
<section class="w-full h-full px-8 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex items-start justify-between mb-8">
            <div>
                <a href="{{ url_for('__BP__.index') }}" class="inline-flex items-center gap-1.5 text-sm text-ink-muted hover:text-brand transition mb-3">
                    <ion-icon name="arrow-back-outline"></ion-icon> __CLASS__
                </a>
                <h1 class="text-2xl font-bold text-ink">__CLASS__ detail</h1>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ url_for('__BP__.edit', item_id=item.id) }}"
                   class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-line text-sm font-medium hover:bg-line transition">
                ...
                <form method="POST" action="{{ url_for('__BP__.delete', item_id=item.id) }}"
                    ...
                    <input type="hidden" name="csrf_token" value="{{ csrf_token() }}">
                    <button type="submit"
                            class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-danger/40 text-danger text-sm font-medium hover:bg-danger/10 transition">
        ...
        <dl class="rounded-xl border border-line bg-surface overflow-hidden mb-8">
            {% for col in columns %}
            <div class="grid grid-cols-3 gap-4 px-6 py-4 border-b border-line">
                <dt class="text-sm font-medium text-ink-muted">{{ col.label }}</dt>
                <dd class="col-span-2 text-sm text-ink">{{ item[col.name] }}</dd>
            </div>
            {% endfor %}
        </dl>

        {% if relationships %}
        <h2 class="text-base font-semibold text-ink mb-4">Related records</h2>
        <div class="space-y-4">
            {% for rel in relationships %}
            <div class="rounded-xl border border-line bg-surface p-6">
                <h3 class="text-sm font-semibold text-ink mb-3">{{ rel.label }}</h3>
                {% set related = item[rel.name] %}
                {% if rel.uselist %}
                    {% if related %}
                    <ul class="space-y-1">
                        {% for obj in related %}
                        <li class="text-sm text-ink-muted">{{ obj[rel.target_label] }}</li>
                        {% endfor %}
                    </ul>
                    {% else %}
                    <p class="text-sm text-ink-muted">No records.</p>
                    {% endif %}
                {% else %}
                    <p class="text-sm text-ink-muted">{{ related[rel.target_label] if related else "" }}</p>
                {% endif %}
            </div>
            {% endfor %}
        </div>
...
<section class="w-full h-full px-8 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-bold text-ink">{% if mode == 'new' %}New __CLASS__{% else %}Edit __CLASS__{% endif %}</h1>
        </div>

        <form method="POST"
              action="{% if mode == 'new' %}{{ url_for('__BP__.create') }}{% else %}{{ url_for('__BP__.update', item_id=item.id) }}{% endif %}"
              class="rounded-xl border border-line bg-surface p-6 space-y-4">
            <input type="hidden" name="csrf_token" value="{{ csrf_token() }}">
            ...
            <div class="flex justify-end gap-3 pt-4">
                <a href="{{ url_for('__BP__.index') }}"
                   class="px-4 py-2.5 rounded-xl border border-line text-sm text-ink-muted hover:bg-line transition">
                    Cancel
                </a>
                <button type="submit"
                        class="px-4 py-2.5 rounded-xl bg-ink text-canvas text-sm font-medium hover:opacity-90 transition">
                    Save
                </button>
            </div>
        </form>
...
def _build_files(info: dict) -> dict:
    ...
    enum_imports = ""
    if used_constants:
        enum_imports = "from application.constants import " + ", ".join(sorted(used_constants))
    ...
    files = {
        ...
        f"blueprints/{module}/services.py": _replace_all(
            _SERVICES_TEMPLATE,
...
def resolve_model(name: str):
    path = introspection.resolve_model_file(name)
    if not path:
        return None
    return introspection.parse_model_file(path)
...
def generate_crud(name: str) -> dict:
    ...
    introspection.sync_models_init()
