...
from application.users import get_user_by_id, mark_user_as_deleted
...
def forgot_password_page():
    return render_template("auth/forgot_password.html", selected="forgot_password")
...
def authenticate():
    ...
    if not res.success:
        ...
        return htmx_response(html="", redirect=url_for("auth_bp.login", next=next_url))

    session.clear()
    session["user_id"] = res.data["user_id"]
    Flash.chat("Welcome!")
    return htmx_response(html="", redirect=next_url)
...
def register_user():
    ...
    if data.get("password") != data.get("confirm_password"):
        ...
        return htmx_response(html="", redirect=url_for("auth_bp.register"))
...
def forgot_password():
    ...
    if res.data:
        ...
        send_email(
            ...
            html=render_template("auth/emails/password_reset.html", url=reset_url),
        )
...
def reset_password_page(user_id, token):
    ...
    res = validate_reset_token(user=user, token=token)
...
def reset_user_password():
    ...
    if password != confirm_password:
        ...
        return redirect(url_for("auth_bp.reset_password_page", user_id=user_id, token=token))

    res = reset_password(user_id=user_id, token=token, new_password=password)

    if not res.success:
        Flash.error(res.message)
        return redirect(url_for("auth_bp.reset_password_page", user_id=user_id, token=token))

    Flash.ok("Password reset successfully. You can now log in.")
    return redirect(url_for("auth_bp.login"))
...
def delete_user():
    ...
    return htmx_response(html="", trigger="usersChange")
...
def logout():
    ...
    if not res.success:
        ...
        return htmx_response(html="", redirect=url_for("core_bp.home"))

    Flash.ok("See you soon!")
    return htmx_response(html="", redirect=url_for("auth_bp.login"))
